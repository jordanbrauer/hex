...
func TestLoad_defaults_perFileNamespace(t *testing.T) {
	...
	if got := s.String("finch.log.level"); got != "info" {
		t.Errorf("finch.log.level = %q, want info", got)
	}
}
...
func TestLoad_userDirOverridesDefaults(t *testing.T) {
	...
	// Override server.port only. finch and database are untouched.
	if err := os.WriteFile(
...
func TestLoad_envMapDottedKeyBindsNestedTOML(t *testing.T) {
	// finch.toml has [log] level; env.yaml binds finch.log.level -> TEST_LOG_LEVEL
	...
	if got := s.String("finch.log.level"); got != "debug" {
		t.Errorf("finch.log.level = %q, want debug (env)", got)
	}
}
...
func TestLoad_envFileLoads(t *testing.T) {
	...
	if got := s.String("finch.log.level"); got != "warn" {
		t.Errorf("finch.log.level = %q, want warn (from .env)", got)
	}
}
...
func TestStore_unmarshalKey(t *testing.T) {
	...
	var got LogCfg
	if err := s.Unmarshal("finch.log", &got); err != nil {
...
func TestStore_namespaces(t *testing.T) {
	...
	got := s.Namespaces()
	want := []string{"database", "finch", "server"}
...
func TestPriorityOrder(t *testing.T) {
	...
	if err := os.WriteFile(
		filepath.Join(userDir, "finch.toml"),
		[]byte("[log]\nlevel = \"warn\"\n"),
		0o644,
	); err != nil {
		t.Fatal(err)
