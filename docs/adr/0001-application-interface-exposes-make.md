...
We considered splitting the interface so `Register()` gets a write-only surface and `Boot()` gets the full surface. Rejected because the unified interface has proven sufficient in real production apps, and the split adds complexity for a problem that hasn't materialised.
