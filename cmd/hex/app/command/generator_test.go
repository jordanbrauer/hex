...
func TestGenFlagsOptions_rejectsUnknownFormat(t *testing.T) {
	if _, err := (genFlags{format: "yaml"}).options(); err == nil {
		t.Errorf("expected an error for --format yaml")
	}
}

func TestGenFlagsOptions_defaultsToText(t *testing.T) {
	f := genFlags{}
